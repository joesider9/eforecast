...
class SemiFullCompressor:
    ...
    def semi_full_compressor(self, data, ax):
        semi_full_type = self.semi_full_type(ax)
        if semi_full_type == 'coarse':
            names, data_compressed = self.semi_full_coarse(data)
        elif semi_full_type == 'reduced':
            names, data_compressed = self.semi_full_reduced(data)
        elif semi_full_type == 'detailed':
            names, data_compressed = self.semi_full_detailed(data)
        else:
            raise ValueError('Unknown semi_full_type')
        return names, data_compressed

    def perform_semi_full_compress(self, i, ax, nwp_data):
        ax_name = '_'.join(ax)
        names, data = self.semi_full_compressor(nwp_data[:, i, :, :], ax)
        data = pd.DataFrame(data, index=self.nwp_metadata['dates'],
                            columns=[ax_name + '_' + name for name in names])
        return data


    def semi_full_type(self, ax):
        ...
        if (var_name == 'WS' and self.type == 'wind') or (var_name == 'Flux' and self.type == 'pv'):
            if (int(lag) == int(lags[0]) or int(lag) == int(lags[-1])) and self.use_data_before_and_after_target:
                return 'coarse'
            else:
                return 'detailed'
    ...
    def semi_full_reduced(self, data):
        if self.static_data['regional']:
            ...
            if data.shape[-1] == 5 and data.shape[-2] == 5:
                ...
                data_semi_full = np.hstack((data_center, data_close, data_around))
                names += ['pcl_' + str(pcl) for pcl in [5, 50, 95]]
                return names, data_semi_full
            else:
                raise ValueError('The size of nwp data should be 5x5')

    def semi_full_detailed(self, data):
        if self.static_data['regional']:
            shape = data.shape
            data_flat = data.reshape(-1, np.prod(shape[1:]))
            data_semi_full = np.hstack(
                (np.mean(data, axis=-1), np.percentile(data_flat, [5, 25, 50, 75, 95], axis=1).T))
            names = [f'mean_{i}' for i in range(shape[-2])] + ['pcl_' + str(pcl) for pcl in [5, 25, 50, 75, 95]]
            return names, data_semi_full
        else:
            if data.shape[-1] == 5 and data.shape[-2] == 5:
                ind = np.array([[1, 1], [1, 2], [1, 3],
                ...
                data_semi_full = np.hstack((data_center, data_around))
                names += ['pcl_' + str(pcl) for pcl in [5, 25, 50, 75, 95]]
                return names, data_semi_full
            else:
                raise ValueError('The size of nwp data should be 5x5')

    def semi_full_coarse(self, data):
        if self.static_data['regional']:
            shape = data.shape
            data = data.reshape(-1, np.prod(shape[1:]))
            semi_full_data = np.hstack((np.mean(data, axis=1).reshape(-1, 1), np.percentile(data, [5, 50, 95], axis=1).T))
            names = ['mean'] + ['pcl_' + str(pcl) for pcl in [5, 50, 95]]
            return names, semi_full_data
        else:
            if data.shape[-1] == 5 and data.shape[-2] == 5:
                centered = data[:, 2, 2].reshape(-1, 1)
                shape = data.shape
                data = data.reshape(-1, np.prod(shape[1:]))
                semi_full_data = np.hstack((centered, np.percentile(data, [5, 50, 95], axis=1).T))
                names = ['center'] + ['pcl_' + str(pcl) for pcl in [5, 50, 95]]
                return names, semi_full_data
            else:
                raise ValueError('The size of nwp data should be 5x5')

    def semi_full_compress(self):
        ...
        if len(groups) == 0:
            ...
            for var_id in range(len(axis)):
                for i, ax in enumerate(axis[var_id]):
                    data = self.perform_semi_full_compress(i, ax, self.nwp_data[..., var_id])
            ...
            for group in groups:
                ...
                for var_id in range(len(axis[group_name])):
                    for i, ax in enumerate(axis[group_name][var_id]):
                        data = self.perform_semi_full_compress(i, ax, self.nwp_data[group_name][..., var_id])
