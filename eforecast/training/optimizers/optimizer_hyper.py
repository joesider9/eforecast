...
class HyperoptOptimizer:
    ...
    def __init__(self, api_config, random=42):
        ...
        space, self.round_to_values, self.encode = HyperoptOptimizer.get_hyperopt_dimensions(api_config)
    ...
    def get_hyperopt_dimensions(api_config):
        ...
        for param_name in param_list:
            ...
            if param_type == "int":
                ...
                assert param_range is None
                encode[param_name] = {str(i): value for i, value in enumerate(param_values)}
                values = [str(i) for i, value in enumerate(param_values)]
        ...
        return space, round_to_values, encode
    ...
    def _suggest(self):
        ...
        seed = np.random.seed(self.random)
    ...
    def suggest(self, n_suggestions=1):
        ...
        for trial in new_trials:
            ...
            X.append(self.decode_cat(x_guess))

        assert len(X) == n_suggestions
        return X

    def decode_cat(self, x):
        for param, values in self.encode.items():
            x[param] = values[x[param]]
        return x


    def encode_cat(self, x):
        for param, values in self.encode.items():
            x[param] = [id for id, value in values.items() if value == x[param]][0]
        return x

    def observe(self, X_org, y_org):
        ...
        y = []
        i = 0
        X_org = X_org.to_dict(orient='index')
        for trial, y1 in zip(X_org.keys(), y_org):
            x = X_org[trial]
            x_guess = tpe.suggest([i], self.domain, self.trials, None)
            x = self.encode_cat(x)
            x_list = {p: [v] for p, v in x.items()}
            x_guess[0]["misc"]["vals"] = x_list

            # Build lookup to get original trial object
            x_guess_ = HyperoptOptimizer.hashable_dict(self.cleanup_guess(x_guess[0]["misc"]["vals"]))
            if x_guess_ not in self.trial_id_lookup:
                self.trials.insert_trial_docs(x_guess)
                self.trials.refresh()
                self.trial_id_lookup[x_guess_] = i
                i += 1
                X.append(self.cleanup_guess(x_guess[0]["misc"]["vals"]))
                y.append(y1)
            else:
                i_old = self.trial_id_lookup[x_guess_]
                if y[i_old] > y1:
                    y[i_old] = y1
        assert len(X) == len(y)
