...
from eforecast.deep_models.pytorch_2x.ts_transformers.layers.SelfAttention_Family import AttentionLayer, FullAttention
...
class Encoder(nn.Module):
    ...
    def __init__(self, 
        ...
        self.layers = nn.ModuleList([
            EncoderLayer(self.d_model, self.d_ff, self.n_heads, dropout=self.dropout,
                         normalize_before=False) for _ in range(self.e_layers)
        ])  # naive pyramid attention

        self.enc_embedding = DataEmbedding(
            self.enc_in, self.d_model, self.dropout)
        self.conv_layers = Bottleneck_Construct(
            self.d_model, window_size, d_bottleneck)
