...
from eforecast.deep_models.pytorch_2x.ts_transformers.layers.Crossformer_EncDec import scale_block, Encoder, Decoder, DecoderLayer
from eforecast.deep_models.pytorch_2x.ts_transformers.layers.Embed import PatchEmbedding
from eforecast.deep_models.pytorch_2x.ts_transformers.layers.SelfAttention_Family import AttentionLayer, FullAttention, TwoStageAttentionLayer
from eforecast.deep_models.pytorch_2x.ts_transformers.PatchTST import FlattenHead
...
class Crossformer(nn.Module):
    def __init__(self, 
        ...
        self.encoder = Encoder(
            [
                scale_block(1 if l == 0 else self.win_size, self.d_model, self.n_heads, self.d_ff,
                            1, self.dropout,
                            self.in_seg_num if l == 0 else ceil(self.in_seg_num / self.win_size ** l), self.factor, self.output_attention
