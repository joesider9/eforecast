...
from eforecast.deep_models.pytorch_2x.ts_transformers.modules.attention_modules import *
from eforecast.deep_models.pytorch_2x.ts_transformers.modules.positional_encoding import PositionalEncoding2D
...
class AxialRotaryEmbedding(nn.Module):
    def __init__(self, dim, freq_type="lucidrains", **kwargs):
        ...
        if freq_type == "lucidrains":
            scales = torch.linspace(1.0, 64 / 2, self.dim // 4)
...
from eforecast.deep_models.pytorch_2x.ts_transformers.modules.positional_encoding import Cyclical_embedding
class RoCrossViViT(nn.Module):
    def __init__(
        ...
        heads: int = 4,
        mlp_ratio: int = 4,
        ctx_channels: int = 9, # n_ctx_channels, n_opt_channels / 2, elevation
        ts_channels: int = 8, # n_ts_channels, elevation
        ts_length: int = 24,
        out_dim: int = 1,
        dim_head: int = 64,
        dropout: float = 0.0,
        ...
        num_mlp_heads: int = 5,
        use_glu: bool = True,
        ctx_masking_ratio: float = 0.5,
        ts_masking_ratio: float = 0.5,
        decoder_dim: int = 128,
        decoder_depth: int = 4,
        decoder_heads: int = 6,
        ...
        **kwargs,
        ...
        self.time_coords_encoder = Cyclical_embedding([12, 31, 24, 60])
