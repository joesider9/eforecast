from eforecast.deep_models.tf_2x.unet.activations import GELU, Snake
import tensorflow as tf
from tensorflow.python.keras.layers import ReLU, LeakyReLU, PReLU, ELU, Softmax
...
def decode_layer(X, channel, pool_size, unpool, kernel_size=3,
    ...
    * The defaut: `kernel_size=3`, is suitable for `pool_size=2`.
    ...
    if unpool:
        X = tf.keras.layers.UpSampling2D(size=(pool_size, pool_size),
                                         interpolation=interp, name='{}_unpool'.format(name))(X)
    else:
        if kernel_size == 'auto':
        ...
        # batch normalization
        if batch_norm:
            X = tf.keras.layers.BatchNormalization(axis=3, name='{}_bn'.format(name))(X)
...
def encode_layer(X, channel, pool_size, pool, kernel_size='auto',
    ...
    if pool == 'max':
        ...
        X = tf.keras.layers.AveragePooling2D(pool_size=(pool_size, pool_size), name='{}_avepool'.format(name))(X)
        ...
        X = tf.keras.layers.Conv2D(channel, kernel_size, strides=(pool_size, pool_size),
...
def attention_gate(X, g, channel,
    ...
    theta_att = tf.keras.layers.Conv2D(channel, 1, use_bias=True, name='{}_theta_x'.format(name))(X)

    # mapping the gate tensor
    phi_g = tf.keras.layers.Conv2D(channel, 1, use_bias=True, name='{}_phi_g'.format(name))(g)

    # ----- attention learning ----- #
    if attention == 'add':
        query = tf.keras.layers.add([theta_att, phi_g], name='{}_add'.format(name))
    else:
        query = tf.keras.layers.multiply([theta_att, phi_g], name='{}_add'.format(name))

    # nonlinear activation
    f = activation_func(name='{}_activation'.format(name))(query)

    # linear transformation
    psi_f = tf.keras.layers.Conv2D(1, 1, use_bias=True, name='{}_psi_f'.format(name))(f)
    # ------------------------------ #

    # sigmoid activation as attention coefficients
    coef_att = tf.keras.layers.Activation('sigmoid', name='{}_sigmoid'.format(name))(psi_f)
...
def CONV_stack(X, channel, kernel_size=3, stack_num=2,
    ...
    for i in range(stack_num):
        ...
        # linear convolution
        X = tf.keras.layers.Conv2D(channel, kernel_size, padding='same', use_bias=bias_flag,
                   dilation_rate=dilation_rate, name='{}_{}'.format(name, i))(X)

        # batch normalization
        if batch_norm:
            X = tf.keras.layers.BatchNormalization(axis=3, name='{}_{}_bn'.format(name, i))(X)
...
def Res_CONV_stack(X, X_skip, channel, res_num, activation='ReLU', batch_norm=False, name='res_conv'):
    ...
    X = tf.keras.layers.add([X_skip, X], name='{}_add'.format(name))
...
def Sep_CONV_stack(X, channel, kernel_size=3, stack_num=1, dilation_rate=1, activation='ReLU', batch_norm=False,
    ...
    for i in range(stack_num):
        X = tf.keras.layers.DepthwiseConv2D(kernel_size, dilation_rate=dilation_rate, padding='same',
                            use_bias=bias_flag, name='{}_{}_depthwise'.format(name, i))(X)

        if batch_norm:
            X = tf.keras.layers.BatchNormalization(name='{}_{}_depthwise_BN'.format(name, i))(X)

        X = activation_func(name='{}_{}_depthwise_activation'.format(name, i))(X)

        X = tf.keras.layers.Conv2D(channel, (1, 1), padding='same', use_bias=bias_flag, name='{}_{}_pointwise'.format(name, i))(X)

        if batch_norm:
            X = tf.keras.layers.BatchNormalization(name='{}_{}_pointwise_BN'.format(name, i))(X)
...
def ASPP_conv(X, channel, activation='ReLU', batch_norm=True, name='aspp'):
    ...
    * dilation rates are fixed to `[6, 9, 12]`.
    ...
    b4 = tf.keras.layers.GlobalAveragePooling2D(name='{}_avepool_b4'.format(name))(X)

    b4 = tf.expand_dims(tf.expand_dims(b4, 1), 1, name='{}_expdim_b4'.format(name))

    b4 = tf.keras.layers.Conv2D(channel, 1, padding='same', use_bias=bias_flag,
                                name='{}_conv_b4'.format(name))(b4)

    if batch_norm:
        b4 = tf.keras.layers.BatchNormalization(name='{}_conv_b4_BN'.format(name))(b4)

    b4 = activation_func(name='{}_conv_b4_activation'.format(name))(b4)

    # <----- tensorflow v1 resize.
    b4 = tf.image.resize(b4, shape_before[1:3], method='bilinear', align_corners=True, name='{}_resize_b4'.format(name))

    b0 = tf.keras.layers.Conv2D(channel, (1, 1), padding='same', use_bias=bias_flag,
                                name='{}_conv_b0'.format(name))(X)

    if batch_norm:
        b0 = tf.keras.layers.BatchNormalization(name='{}_conv_b0_BN'.format(name))(b0)
    ...
    return tf.concat([b4, b0, b_r6, b_r9, b_r12])
...
def CONV_output(X, n_labels, kernel_size=1, activation='Softmax', name='conv_output'):
    ...
    ----------
        ...
        activation: one of the `tensorflow.keras.layers` or `eforecast.deep_models.tf_2x.unet.activations` interface or 'Sigmoid'.
    ...
    if activation:
        ...
        if activation == 'Sigmoid':
            X = tf.keras.layers.Activation('sigmoid', name='{}_activation'.format(name))(X)
