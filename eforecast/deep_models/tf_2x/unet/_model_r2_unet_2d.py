...
def RR_CONV(X, channel, kernel_size=3, stack_num=2, recur_num=2, activation='ReLU', batch_norm=False, name='rr'):
    ...
    layer_skip = tf.keras.layers.Conv2D(channel, 1, name='{}_conv'.format(name))(X)
    layer_main = layer_skip

    for i in range(stack_num):

        layer_res = tf.keras.layers.Conv2D(channel, kernel_size, padding='same', name='{}_conv{}'.format(name, i))(
            layer_main)

        if batch_norm:
            layer_res = tf.keras.layers.BatchNormalization(name='{}_bn{}'.format(name, i))(layer_res)

        layer_res = activation_func(name='{}_activation{}'.format(name, i))(layer_res)

        for j in range(recur_num):

            layer_add = tf.keras.layers.add([layer_res, layer_main], name='{}_add{}_{}'.format(name, i, j))

            layer_res = tf.keras.layers.Conv2D(channel, kernel_size, padding='same',
                                               name='{}_conv{}_{}'.format(name, i, j))(layer_add)

            if batch_norm:
                layer_res = tf.keras.layers.BatchNormalization(name='{}_bn{}_{}'.format(name, i, j))(layer_res)

            layer_res = activation_func(name='{}_activation{}_{}'.format(name, i, j))(layer_res)

        layer_main = layer_res

    out_layer = tf.keras.layers.add([layer_main, layer_skip], name='{}_add{}'.format(name, i))
...
def UNET_RR_right(X, X_list, channel, kernel_size=3,
    ...
    H = tf.concat([X, ] + X_list, axis=-1, name='{}_concat'.format(name))
...
def r2_unet_2d(input_size=None, filter_num=None, n_labels=32,
    ...
    IN = tf.keras.Input(input_size, name='{}_input'.format(name))
