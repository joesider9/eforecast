...
from eforecast.deep_models.tf_2x.unet.utils import freeze_model
...
def backbone_zoo(backbone_name, weights, input_tensor, depth, freeze_backbone, freeze_batch_norm):
    ...
    backbone_func = eval(f'tf.keras.applications.{backbone_name}')
    ...
    model = tf.keras.Model(inputs=[input_tensor,], outputs=X_skip, name='{}_backbone'.format(backbone_name))
