...
class patch_extract(tf.keras.layers.Layer):
    '''
...
class patch_merging(tf.keras.layers.Layer):
    ...
    def __init__(self, num_patch, embed_dim, name='', **kwargs):
        ...
        self.linear_trans = tf.keras.layers.Dense(2 * embed_dim, use_bias=False, name='{}_linear_trans'.format(name))
...
class patch_expanding(tf.keras.layers.Layer):
    ...
    def __init__(self, num_patch, embed_dim, upsample_rate, return_vector=True, name='patch_expand', **kwargs):
        ...
        self.linear_trans1 = tf.keras.layers.Conv2D(upsample_rate * embed_dim, kernel_size=1, use_bias=False,
                                    name='{}_linear_trans1'.format(name))
        # 
        self.linear_trans2 = tf.keras.layers.Conv2D(upsample_rate * embed_dim, kernel_size=1, use_bias=False,
                                    name='{}_linear_trans1'.format(name))
        self.prefix = name
...
class drop_path(tf.keras.layers.Layer):
...
class Mlp(tf.keras.layers.Layer):
    def __init__(self, filter_num, drop=0., name='mlp', **kwargs):
        ...
        # MLP layers
        self.fc1 = tf.keras.layers.Dense(filter_num[0], name='{}_mlp_0'.format(name))
        self.fc2 = tf.keras.layers.Dense(filter_num[1], name='{}_mlp_1'.format(name))
...
class WindowAttention(tf.keras.layers.Layer):
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None,
        ...
        # Layers
        self.qkv = tf.keras.layers.Dense(dim * 3, use_bias=qkv_bias, name='{}_attn_qkv'.format(self.prefix))
        self.attn_drop = tf.keras.layers.Dropout(attn_drop)
        self.proj = tf.keras.layers.Dense(dim, name='{}_attn_proj'.format(self.prefix))
...
class SwinTransformerBlock(tf.keras.layers.Layer):
    def __init__(self, dim, num_patch, num_heads, window_size=7, shift_size=0,
        ...
        # Layers
        self.norm1 = tf.keras.layers.LayerNormalization(epsilon=1e-5, name='{}_norm1'.format(self.prefix))
        ...
        self.norm2 = tf.keras.layers.LayerNormalization(epsilon=1e-5, name='{}_norm2'.format(self.prefix))
