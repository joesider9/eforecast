...
def ResUNET_a_block(X, channel, kernel_size=3, dilation_num=1.0, activation='ReLU', batch_norm=False,
                    name='res_a_block'):
    '''
    The "ResUNET-a" block
    ...
    if len(X_res) > 1:
        return tf.keras.layers.add(X_res)
...
def ResUNET_a_right(X, X_list, channel, kernel_size=3, dilation_num=[1, ],
    ...
    X = tf.concat([X, ] + X_list, axis=3, name=name + '_concat')
...
def resunet_a_2d_base(input_tensor, filter_num, dilation_num,
    ...
    * `dilation_num` should be provided as 2d iterables, with the second dimension matches the model depth.
    ...
    X = tf.keras.layers.Conv2D(filter_num[0], 1, 1, dilation_rate=1, padding='same',
    ...
    X = tf.concat([X_skip[-1], X], name='{}_concat_out'.format(name))
...
def resunet_a_2d(input_size, filter_num, dilation_num=None, n_labels=12,
    ...
    * `resunet_a_2d` does not support NoneType input shape.
    ...
    model = tf.keras.Model([IN], [OUT, ], name='{}_model'.format(name))
