...
from eforecast.deep_models.tf_2x.unet.layer_utils import *
...
def RSU(X, channel_in, channel_out, depth=5, activation='ReLU', batch_norm=True, pool=True, unpool=True, name='RSU'):
    ...
    ----------
        ...
        activation: one of the `tensorflow.keras.layers` or `eforecast.deep_models.tf_2x.unet.activations` interfaces, e.g., 'ReLU'.
    ...
    for i in range(depth):
        X = tf.concat([X, X_skip[i]], axis=-1, name='{}_concat_{}'.format(name, i))
    ...
    X = tf.concat([X, X_skip[depth]], axis=-1, name='{}_concat_out'.format(name))
    ...
    X = tf.keras.layers.add([X, X_skip[-1]], name='{}_out_add'.format(name))
    return X
...
def RSU4F(X, channel_in, channel_out, dilation_num=[1, 2, 4, 8], activation='ReLU', batch_norm=True, name='RSU4F'):
    ...
    X = tf.concat([X, X_skip[2]], axis=-1, name='{}_concat_out'.format(name))
    X = CONV_stack(X, channel_out, kernel_size=3, stack_num=1, dilation_rate=1,
                   activation=activation, batch_norm=batch_norm, name='{}_out'.format(name))

    return tf.keras.layers.add([X, X_skip[-1]], name='{}_out_add'.format(name))
...
def u2net_2d_base(input_tensor,
    ...
    * Upsampling is achieved through bilinear interpolation in Qin et al. (2020), 
    ...
    for i, f in enumerate(filter_4f_num):
        ...
        X = tf.concat([X, X_skip[tensor_count]], axis=-1, name='{}_concat_4f_{}'.format(name, i))
    ...
    for i, f in enumerate(filter_num_up):
        ...
        X = tf.concat([X, X_skip[tensor_count]], axis=-1, name='{}_concat_{}'.format(name, i))
...
def u2net_2d(input_size=None, filter_num=None, n_labels=32,
    ...
    ----------
        ...
        filter_4f_mid_num: a list that defines the number of RSU-4F intermediate filters for each 
                           ...
                           * RSU-4F intermediate filters numbers are expected to be smaller than output filters numbers.         
        activation: one of the `tensorflow.keras.layers` or `eforecast.deep_models.tf_2x.unet.activations` interfaces, e.g., 'ReLU'.
        output_activation: one of the `tensorflow.keras.layers` or `eforecast.deep_models.tf_2x.unet.activations` interface or 'Sigmoid'.
    ...
    * Upsampling is achieved through bilinear interpolation and can be replaced by transpose convolutional layers.
    ...
    if verbose:
        ...
        print(
            '\tNumber of RSU output channels within downsampling blocks: filter_num_down = {}'.format(filter_num_down))
        print('\tNumber of RSU intermediate channels within downsampling blocks: filter_mid_num_down = {}'.format(
            filter_mid_num_down))
        print('\tNumber of RSU output channels within upsampling blocks: filter_num_up = {}'.format(filter_num_up))
        print('\tNumber of RSU intermediate channels within upsampling blocks: filter_mid_num_up = {}'.format(
            filter_mid_num_up))
        print('\tNumber of RSU-4F output channels within downsampling and bottom blocks: filter_4f_num = {}'.format(
            filter_4f_num))
        print(
    ...
    IN = tf.keras.Input(shape=input_size)
    ...
    for i in range(1, L_out):
        ...
        X = tf.keras.layers.Conv2D(n_labels, 3, padding='same', name='{}_output_conv_{}'.format(name, i))(X_out[i])
        ...
        if output_activation:
            if output_activation == 'Sigmoid':
                X = tf.keras.layers.Activation('sigmoid', name='{}_output_sup{}_activation'.format(name, i))(X)
            else:
    ...
    D = tf.concat(OUT_stack, axis=-1, name='{}_output_concat'.format(name))
    ...
    if deep_supervision:
        ...
        model = tf.keras.Model([IN, ], OUT_stack)

    else:
        model = tf.keras.Model([IN, ], [D, ])
