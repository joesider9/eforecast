...
from eforecast.deep_models.tf_2x.unet._model_unet_2d import UNET_left, UNET_right
from eforecast.deep_models.tf_2x.unet.transformer_layers import patch_extract, patch_embedding
from eforecast.deep_models.tf_2x.unet._backbone_zoo import backbone_zoo, bach_norm_checker
...
def ViT_MLP(X, filter_num, activation='GELU', name='MLP'):
    ...
    for i, f in enumerate(filter_num):
        X = tf.keras.layers.Dense(f, name='{}_dense_{}'.format(name, i))(X)
...
def ViT_block(V, num_heads, key_dim, filter_num_MLP, activation='GELU', name='ViT'):
    ...
    V_atten = V  # <--- skip
    V_atten = tf.keras.layers.LayerNormalization(name='{}_layer_norm_1'.format(name))(V_atten)
    V_atten = tf.keras.layers.MultiHeadAttention(num_heads=num_heads, key_dim=key_dim,
    ...
    # MLP
    V_MLP = V_add  # <--- skip
    V_MLP = tf.keras.layers.LayerNormalization(name='{}_layer_norm_2'.format(name))(V_MLP)
    V_MLP = ViT_MLP(V_MLP, filter_num_MLP, activation, name='{}_mlp'.format(name))
    # Skip connection
    V_out = tf.keras.layers.add([V_MLP, V_add], name='{}_skip_2'.format(name))  # <--- skip
...
def transunet_2d_base(input_tensor, filter_num, stack_num_down=2, stack_num_up=2,
    ...
    X = tf.keras.layers.Conv2D(filter_num[-1], 1, padding='valid', use_bias=False,
...
def transunet_2d(input_size=None, filter_num=None, n_labels=32, stack_num_down=2, stack_num_up=2,
                 embed_dim=768, num_mlp=3072, num_heads=4, num_transformer=2,
    ...
    model = tf.keras.Model(inputs=[IN, ], outputs=[OUT, ], name='{}_model'.format(name))
