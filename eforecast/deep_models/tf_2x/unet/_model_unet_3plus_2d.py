...
def unet_3plus_2d_base(input_tensor, filter_num_down, filter_num_skip, filter_num_aggregate,
    ...
    * Upsampling is achieved through bilinear interpolation and can be replaced by transpose convolutional layers here.
    ...
    for i in range(depth_decode):
        ...
        X = tf.concat(X_fscale, axis=-1, name='{}_concat_{}'.format(name, i))
...
def unet_3plus_2d(input_size=None, filter_num=None, n_labels=32, filter_num_skip='auto', filter_num_aggregate='auto',
    ...
    * Upsampling is achieved through bilinear interpolation and can be replaced by transpose convolutional layers here.
    ...
    if deep_supervision:
        ...
        if ('{}_backbone_'.format(backbone) in X_decoder[0].name) and freeze_backbone:
            backbone_warn = '\n\nThe deepest UNET 3+ deep supervision branch directly connects to a frozen backbone.\nTesting your configurations on `eforecast.deep_models.tf_2x.unet.base.unet_plus_2d_base` is recommended.'
            print(backbone_warn);
        # ----------------------------------------- #
        ...
        for i in range(1, L_out):
            ...
            X = tf.keras.layers.Conv2D(n_labels, 3, padding='same', name='{}_output_conv_{}'.format(name, i - 1))(
                X_decoder[i])
            ...
            if output_activation:
                ...
                if output_activation == 'Sigmoid':
                    X = tf.keras.layers.Activation('sigmoid', name='{}_output_sup{}_activation'.format(name, i - 1))(X)
                else:
