...
class Leshy(SelectorMixin, BaseEstimator):
    ...
    def _fit(self, X_raw, y, sample_weight=None):
        ...
        # X = X.fillna(0)
        multi_output = y.shape[1] > 1


        # check input params
        self._check_params(X, y, multi_output)
        ...
        if self.n_estimators != "auto":
            self.estimator.set_params(n_estimators=self.n_estimators, silent=True)
    ...
    def _check_params(self, X, y, multi_output):
...
def _get_shap_imp(estimator, X, y, sample_weight=None, cat_feature=None):
    ...
    if is_lightgbm(estimator):
        ...
        if is_classifier(model):
            ...
            if isinstance(shap_values, list):
                shap_imp = np.abs(shap_values).mean(0).mean(0)
            else:
